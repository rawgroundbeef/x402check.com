...
must_haves:
  ...
  artifacts:
    - path: "packages/x402check/src/validation/orchestrator.ts"
    ...
    - path: "packages/x402check/src/index.ts"
      ...
      exports: ["validate"]
  key_links:
    - from: "packages/x402check/src/validation/orchestrator.ts"
      to: "packages/x402check/src/validation/rules/index.ts"
      via: "imports all rule validators"
      pattern: "import.*from.*rules"
    - from: "packages/x402check/src/validation/orchestrator.ts"
      to: "packages/x402check/src/detection/normalize.ts"
      via: "calls normalize() on parsed input"
      pattern: "import.*normalize"
    - from: "packages/x402check/src/validation/orchestrator.ts"
      to: "packages/x402check/src/validation/address.ts"
...
<task type="auto">
  ...
  <action>
Create `packages/x402check/src/validation/orchestrator.ts` implementing the validate() function.
...
<task type="auto">
  ...
  <action>
**Update `packages/x402check/src/validation/index.ts`:**
...
**Update `packages/x402check/src/index.ts`:**
...
Keep all existing exports intact.
  ...
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && npx tsc --noEmit -p packages/x402check/tsconfig.json` -- no type errors. Verify `validate` is exported by checking: `grep -n 'export.*validate' packages/x402check/src/index.ts`.
  </verify>
  <done>
validate() is exported from SDK entry point. `import { validate } from 'x402check'` will work once built. ValidationOptions type is also exported for TypeScript consumers.
  </done>
</task>
...
2. `grep 'export.*validate' packages/x402check/src/index.ts` shows validate export
