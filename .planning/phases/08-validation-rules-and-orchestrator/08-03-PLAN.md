...
must_haves:
  ...
  artifacts:
    - path: "packages/x402check/test/validation/rules/structure.test.ts"
      provides: "Structure rule unit tests"
      min_lines: 30
    - path: "packages/x402check/test/validation/rules/fields.test.ts"
      provides: "Fields rule unit tests"
      min_lines: 50
    - path: "packages/x402check/test/validation/rules/network.test.ts"
      provides: "Network rule unit tests with fix suggestion checks"
      min_lines: 40
    - path: "packages/x402check/test/validation/rules/amount.test.ts"
      provides: "Amount rule unit tests"
      min_lines: 30
    - path: "packages/x402check/test/validation/orchestrator.test.ts"
      provides: "Orchestrator integration tests covering full pipeline"
      min_lines: 100
    - path: "packages/x402check/test/integration.test.ts"
      provides: "End-to-end tests with JSON fixtures and round-trips"
      min_lines: 80
    - path: "packages/x402check/test/fixtures/valid-v2-base.json"
      provides: "Valid v2 config fixture for Base network"
    - path: "packages/x402check/test/fixtures/valid-v1.json"
      provides: "Valid v1 config fixture"
  key_links:
    - from: "packages/x402check/test/validation/orchestrator.test.ts"
      to: "packages/x402check/src/validation/orchestrator.ts"
      via: "imports validate()"
      pattern: "import.*validate.*orchestrator"
    - from: "packages/x402check/test/integration.test.ts"
      to: "packages/x402check/test/fixtures/"
...
Purpose: The test suite proves the validate() API works correctly for all input scenarios. Every error code must be exercised. JSON fixtures enable reproducible testing and serve as documentation of expected behavior. The 100+ test case target ensures comprehensive coverage.
...
<task type="auto">
  ...
  <action>
**JSON Fixtures** (create in `packages/x402check/test/fixtures/`):
...
**Rule Unit Tests** (create in `packages/x402check/test/validation/rules/`):
...
Target: ~50 unit tests across 6 files.
  </action>
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && npx vitest run --reporter=verbose 2>&1 | grep -c 'PASS\|✓\|pass'` to count passing tests. All rule unit tests pass.
  </verify>
  <done>
7 JSON fixture files created. 6 rule unit test files with ~50 tests total, all passing. Every error code from structure, version, fields, network, amount, and legacy rules is exercised.
  </done>
</task>
...
**Target: combined with Task 1's ~50 unit tests, reach 100+ total.**
Count tests by running: `npx vitest run --reporter=verbose 2>&1 | tail -5` which shows total test count.
  </action>
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && npx vitest run` -- all tests pass with zero failures. Run `npx vitest run --reporter=verbose 2>&1 | grep -c '✓'` to verify 100+ test cases. If under 100, add more edge cases to orchestrator.test.ts.
  </verify>
  <done>
100+ test cases pass. Every ErrorCode is exercised. JSON fixtures load and validate correctly. Round-trip tests (normalize then validate) pass. Strict mode tests confirm warning promotion. Full integration coverage of the validate() pipeline.
  </done>
</task>
...
<verification>
1. `cd /Users/rawgroundbeef/Projects/x402check && npx vitest run` -- all tests pass, zero failures
2. Test count >= 100 (verified via verbose output)
3. Every ErrorCode constant appears in at least one test assertion
...
- Every ErrorCode has at least one test producing it
