...
must_haves:
  ...
  artifacts:
    - path: "apps/website/index.html"
      ...
      contains: "x402check@0.0.1/dist/index.iife.js"
    - path: "apps/website/input.js"
      provides: "Adapter layer mapping SDK validate() to old validateX402Config() shape"
      contains: "window.x402Validate.validate"
  key_links:
    - from: "apps/website/index.html"
      to: "SDK IIFE bundle"
      via: "script tag loading cdn.jsdelivr.net/npm/x402check@0.0.1/dist/index.iife.js"
      pattern: "jsdelivr.*x402check.*index\\.iife\\.js"
    - from: "apps/website/input.js"
      to: "window.x402Validate.validate"
      via: "adapter function validateX402Config calls SDK"
      pattern: "x402Validate\\.validate"
...
@packages/x402check/dist/index.iife.js
</context>
...
With 2 script tags:
```html
<script src="https://cdn.jsdelivr.net/npm/x402check@0.0.1/dist/index.iife.js" crossorigin="anonymous"></script>
<script src="input.js"></script>
```
...
The adapter must:
1. Call `window.x402Validate.validate(configText)` to get the SDK result
...
Also keep the `generateV2Equivalent()` function -- move it to input.js or inline in index.html. Update it to use the SDK's normalize function: `window.x402Validate.normalize(config)` returns a NormalizedConfig with `accepts[]` array. Map back to the old "v2" shape for display.
...
3. Check `window.x402Validate` exists and has `.validate` function
...
- validateX402Config() adapter in input.js calls window.x402Validate.validate() and maps result
...
- Mention this is powered by the x402check SDK
...
3. In console: `typeof window.x402Validate.validate === 'function'` returns true
